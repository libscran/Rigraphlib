...
If the `RIGRAPHLIB_USE_SYSTEM_LIBRARY` environment variable is set to 1,
`pkgconfig()` will instead call `pkg-config igraph` to determine the relevant flags to link to the system **igraph** installation.
In addition, if this environment variable is set during `r self` installation,
the vendored source code will not be compiled unless the `RIGRAPH_FORCE_BUILD` environment variable is set to 1.
